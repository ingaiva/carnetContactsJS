// Script simple sans rechargement du tableau
...
function createRowContact(objContact){
    if(objContact.cIsValid()){        
        ...
        btnIsActif.addEventListener("click", function (e) {            
            ...
            newRow.style.backgroundColor = getBgColor(objContact.cActif); 
            updateLstContact(tdC,objContact);            
        });
...
function  deleteContact(cRow){  
    
    lstContacts.splice(cRow.rowIndex -1 ,1);
...
function updateLstContact(tdC, objContact){
    const idx = lstContacts.findIndex(item => item.cId === objContact.cId);    
    lstContacts[idx]=objContact;     
    // lstContacts[tdC.parentNode.rowIndex -1 ]=objContact;     
    localStorage.setItem("contacts", JSON.stringify(lstContacts));  
}
...
// let btnUpdate = document.createElement("a");        
        // btnUpdate.title="modifier";
        // btnUpdate.insertAdjacentHTML('beforeend', '<i class="fas fa-user-edit"></i>'); 
        // btnUpdate.style.color = "blue";
        // btnUpdate.classList.add("mx-2");
        // btnUpdate.classList.add("btn-maj");
        // btnUpdate.style.cursor = "pointer";       
        // btnUpdate.addEventListener("click", function (e) {
        //     startUpdateContact(newRow);
        // });
        // tdC.appendChild(btnUpdate);